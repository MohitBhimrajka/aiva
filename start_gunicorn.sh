...
# NUCLEAR OPTION: Production fast start mode
if [ "$PRODUCTION_FAST_START" = "true" ]; then
    echo "⚡ PRODUCTION FAST START MODE ENABLED"
    echo "   🚀 Skipping ALL heavy startup operations"
    echo "   🗃️ Database setup handled by Cloud Build jobs"
    echo "   🎯 Starting web server immediately..."
    
    # Go directly to application startup
...
echo "   PRODUCTION_FAST_START=${PRODUCTION_FAST_START:-'not set'}"
echo "   CLOUD_RUN_SERVICE=${CLOUD_RUN_SERVICE:-'not set'}"
echo "   K_SERVICE=${K_SERVICE:-'not set'}"
echo "   PORT=${PORT:-'not set'}"
echo "   GOOGLE_CLOUD_PROJECT=${GOOGLE_CLOUD_PROJECT:-'not set'}"

# Detect if running in Cloud Run
# Check multiple indicators for Cloud Run environment
if [ "$CLOUD_RUN_SERVICE" = "true" ] || [ -n "$K_SERVICE" ] || ([ "$PORT" = "8000" ] && [ -n "$GOOGLE_CLOUD_PROJECT" ]); then
    echo "🚀 Cloud Run environment detected"
    export IS_CLOUD_RUN=true
    export SKIP_HEAVY_STARTUP=true
else
    echo "💻 Local/Docker environment detected"  
    export IS_CLOUD_RUN=false
    export SKIP_HEAVY_STARTUP=false
fi
...
seed_database() {
    if [ "$SKIP_HEAVY_STARTUP" = "true" ]; then
        echo "🚀 Cloud Run mode - basic seeding only"
        # Only run critical seed scripts
        python scripts/seed_roles.py 2>/dev/null || echo "⚠️ Roles already seeded"
        return 0
    fi
...
seed_test_users() {
    if [ "$SKIP_HEAVY_STARTUP" = "true" ]; then
        echo "⏭️  Cloud Run mode - skipping test user seeding"
        return 0
    fi
...
" 2>/dev/null || echo "40")
    ...
    if [ $MISSING_VIDEOS -gt 0 ]; then
        # Check if we should skip heavy startup operations
        if [ "$SKIP_HEAVY_STARTUP" = "true" ]; then
            echo "🚀 Cloud Run detected - skipping video generation during startup"
            echo "   📊 Missing videos will be generated by Cloud Build jobs"
